$(document).ready(function ($) {
     ...
     */
    // Add an animated transition on the help text
    $('#app_user_workPhoneNumber_help').css({transition : 'all 0.1s ease-in-out'});

    // User is deleting things in the input
    // Reset of the helper classes
    $('[name="app_user[workPhoneNumber]"]').keydown(function (e) {
        if (e.which == 8)
        {
    ...
    /*
     * --------------------------------------------------
     * --- Configure the checker of workFax phone number ---
     * --------------------------------------------------
     */
    // Add an animated transition on the help text
    $('#app_user_homeNumber_help').css({transition : 'all 0.1s ease-in-out'});

    // User is deleting things in the input
    // Reset of the helper classes
    $('[name="app_user[workFaxNumber]"]').keydown(function (e) {
        if (e.which == 8)
        {
            regularizeInputHelper($('#app_user_workFaxNumber_help'));
            return true;
        }
    });
    ...
    // User is quitting the input
    // There is no need anymore of emphasis
    $('[name="app_user[workFaxNumber]"]').focusout(function () {
        regularizeInputHelper($('#app_user_workFaxNumber_help'));
        return true;
    })
    ...
    $('[name="app_user[workFaxNumber]"]').keypress(function (e) {
        ...
        if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57))
        {
            emphaseInputHelper($('#app_user_workFaxNumber_help'));
            e.preventDefault();
            return false;
        }
